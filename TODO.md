...
1. Black and Isort for python code formatting — code is already black-formatted
   (7aa1c58); add both as dev dependencies with config in pyproject.toml so it
   stays that way.
2. What do we want to do with .fit/.tcx files once they've been imported?
   ...
   alongside gpx/.
3. How do we name garmin workout plans so that they're readable on the watch?
...
4. Add scheduling support for garmin workouts, ability to make multiple
...
5. Simple text calendar of days in the last 2 calendar months an activity has
   occured.
6. How can we set up the database to work with a cloud service like
